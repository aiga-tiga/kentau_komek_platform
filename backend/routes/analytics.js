...
// Every KPI/chart on the dashboard excludes archived complaints (e.g. test
// entries) by default, so they can't skew real statistics - see
// PATCH /complaints/:id/archive. category is always $1 when present, so
// every query below can reuse the same params array.
function whereClause(categoryList, extra) {
  const clauses = ["archived = false"];
  if (categoryList.length) clauses.push("category = ANY($1)");
...
router.get("/", requireAuth, async (req, res, next) => {
  try {
    ...
    const statusCounts = await pool.query(
      `SELECT status, COUNT(*)::int AS n FROM complaints ${whereClause(list)} GROUP BY status`,
      params
    );
    ...
    const categoryCounts = await pool.query(
      `SELECT category, COUNT(*)::int AS n FROM complaints ${whereClause(list)} GROUP BY category`,
      params
    );
    ...
    const total = await pool.query(`SELECT COUNT(*)::int AS n FROM complaints ${whereClause(list)}`, params);

    const overdue = await pool.query(
      `SELECT COUNT(*)::int AS n FROM complaints ${whereClause(list, "status = 'in_progress' AND deadline < now()")}`,
      params
    );

    const avgResolution = await pool.query(
      `SELECT AVG(EXTRACT(EPOCH FROM (completed_at - created_at)) / 3600.0) AS hours
       FROM complaints ${whereClause(list, "completed_at IS NOT NULL")}`,
      params
    );
    ...
    const trend = await pool.query(
      `SELECT to_char(date_trunc('${trunc}', created_at), 'YYYY-MM-DD') AS day, COUNT(*)::int AS n
       FROM complaints ${whereClause(list, `created_at > now() - interval '${interval}'`)}
       GROUP BY 1 ORDER BY 1`,
      params
    );

    const mapPoints = await pool.query(
      `SELECT id, code, status, category, address, lat, lng
       FROM complaints ${whereClause(list, "lat IS NOT NULL AND lng IS NOT NULL")}`,
      params
    );
